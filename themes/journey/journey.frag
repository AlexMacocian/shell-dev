// SOURCE: https://www.shadertoy.com/view/ldlcRf
// Adapted for glslViewer: aliases u_resolution/u_time to iResolution/iTime
// so the original Shadertoy mainImage() can stay unchanged. A gl_FragColor
// main() at the bottom of this file calls mainImage().
...
// Adapted: original sampled iChannel0 noise texture; replaced with a
// procedural 2D hash so glslViewer doesn't need an external texture.
//==========================================================================================
...
vec3 noised( in vec2 x )
{
    ...
    vec2 p = floor(x);
    float a = _hash21(p + vec2(0.0, 0.0));
    float b = _hash21(p + vec2(1.0, 0.0));
    float c = _hash21(p + vec2(0.0, 1.0));
    float d = _hash21(p + vec2(1.0, 1.0));
...
{ 
	...
	if( TEST_MAT_LESS (m, MAT_TERRAIN_TRAIL ) )
	{
	    ...
	    // Original sampled iChannel1 (a noise/glitter texture) — replaced
	    // with a procedural 2D hash so we don't need an external texture.
	    // Coarser repetition + lower amplitude + stronger pow exponent
	    // softens the harsh per-pixel sand grain into subtle sparkle.
	    float textureGlitter  = _hash21(pos.xz * _TerrainGlitterRep * 0.42) * 0.58;
	    textureGlitter	= pow(textureGlitter , max(_TerrainGlitterPower * 1.8, 3.6));
...
{
	// Camera held stationary (was: noise-driven drift via _CameraMovement).
	vec3 ro = _CameraPos;
...
// mainImage() writes alpha=0 which the recorder treats as transparent
// (resulting in a fully black mp4), so we force alpha=1 here.
void main()
{
    vec4 color;
